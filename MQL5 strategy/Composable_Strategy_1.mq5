...
//|            "2,4".                                                |
//|                                                                  |
//| ADX parity: engine uses Wilder smoothing (matches MQL5 iADX      |
//| main line, buffer 0).                                             |
//+------------------------------------------------------------------+
#property copyright "Composable Strategy 1"
...
//+------------------------------------------------------------------+
// -- Inputs (defaults = strategy_1 params)
//+------------------------------------------------------------------+
...
input int    InpAdxPeriod   = 6;       // ADX period (H1)
input double InpAdxMax      = 80.0;    // ADX < InpAdxMax condition
...
#define ADX_1 1     // iADX(_Symbol, PERIOD_H1, InpAdxPeriod)
...
int OnInit()
  {
   ...
   //--- Create the built-in ATR + ADX handles on H1
   if(!initIndicators())
      return(INIT_FAILED);
...
void OnTick()
  {
   //--- Do we have enough history for ATR(28)/ADX(6) on H1?
   ...
   //--- Condition: ADX(6) < 80 AND Close < Open -> BUY at ASK.
   if(hour == InpEntryHour + 1)
     {
      ...
      //--- Condition 1: Close < Open (bearish H1 bar)
      if(h1Close >= h1Open)
         return;

      //--- Condition 2: ADX(6) < InpAdxMax on that H1 bar
      double adxVal = sqGetIndicatorBufferValue(ADX_1, ADX_MAIN, h1Shift);
      if(adxVal <= 0.0)
         return;
      if(adxVal >= InpAdxMax)
         return;
      ...
      //--- Send BUY
      if(!g_trade.Buy(size, _Symbol, ask, slPrice, 0.0, "Composable_1"))
