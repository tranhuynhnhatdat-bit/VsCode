//+------------------------------------------------------------------+
//| Composable_Strategy_31.mq5                                        |
//|                                                                  |
//| Faithful MQL5 port of the Python backtesting engine for          |
//| results/strategy_31 (ComposableStrategy).                        |
//|                                                                  |
//| Written in the StrategyQuant X idiom (see Strategy_QuantX_       |
//| reference) but using ONLY built-in MQL5 indicators (iATR).       |
//|                                                                  |
//| Strategy (from results/strategy_31/strategy.md — the source of   |
//| truth):                                                          |
//|   Symbol : XAUUSD (report ran on XAUUSD_QDM / FTMO-Demo)         |
//|   Signal : H1 Close<Open on the 01:00 bar -> BUY, executed on    |
//|            the first M1 tick at/after 02:00 (ASK + spread)       |
//|   Exit   : first M1 tick at/after 23:00, same calendar day the   |
//|            position was opened (CLOSE at BID)                    |
//|   SL     : 1.5 x ATR(17) H1, SL = ask - distance (fixed)         |
...
//|                                                                  |
//| IMPORTANT (see MQL5 strategy/README.md):                         |
//|   MQL5 weekday numbers differ from Python: Wed=3, Fri=5.         |
//|   Default InpSessionDays = "3,5".  Do NOT revert to "2,4".       |
//|                                                                  |
//| Timeframe-agnostic since v1.01: reads the current forming M1 bar |
//| via iTime(_Symbol, PERIOD_M1, 0) instead of CopyRates(_Period).  |
//| Run in the Strategy Tester on ANY period (M1 or H1) with XAUUSD. |
//+------------------------------------------------------------------+
#property copyright "Composable Strategy 31"
...
//+------------------------------------------------------------------+
// -- Inputs (defaults = strategy_31 params)
//+------------------------------------------------------------------+
input double InpRiskMoney   = 100.0;   // Risk money per trade (USD)
input double InpSlAtr       = 1.5;     // SL = InpSlAtr * ATR(InpAtrPeriod)
input int    InpAtrPeriod   = 17;      // ATR period (H1)
...
input int    InpMagicNumber = 31;      // Magic number
...
int OnInit()
  {
   ...
   //--- Create the built-in ATR handle on H1 (the signal timeframe)
   if(!initIndicators())
      return(INIT_FAILED);
...
void OnTick()
  {
   //--- Do we have enough history for ATR(17) on H1?
   if(Bars(_Symbol, PERIOD_H1) < InpAtrPeriod + 10)
      return;
   if(Bars(_Symbol, PERIOD_M1) < 5)
      return;

   //--- Current forming M1 bar (timeframe-agnostic: always M1, regardless
   //--- of the tester chart period). Entry/exit fire on the FIRST tick
   //--- whose M1 bar time is at/after the target hour.
   ...
   //--- Condition: Close < Open on that H1 bar  ->  BUY at ASK.
   if(hour == InpEntryHour + 1)
     {
      ...
      //--- The H1 signal bar opened today at InpEntryHour:00 and closed at
      //--- InpEntryHour+1:00. Build its open time from the calendar date.
      ...
      //--- Condition: Close < Open (bearish H1 bar)
      if(h1Close >= h1Open)
         return;

      //--- SL distance = InpSlAtr * ATR(17) on that H1 bar
      ...
      //--- Send BUY
      if(!g_trade.Buy(size, _Symbol, ask, slPrice, 0.0, "Composable_31"))
...
bool initIndicators()
  {
   ArrayResize(indicatorHandles, 1);
...
//+------------------------------------------------------------------+
//| Read a built-in indicator value at a given shift                 |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Was the open position opened this session (same calendar day)?   |
//+------------------------------------------------------------------+
bool PositionOpenedThisSession(datetime nowTime)
  {
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ulong ticket = PositionGetTicket(i);
      ...
      if(PositionSelectByTicket(ticket))
        {
         if(PositionGetString(POSITION_SYMBOL) == _Symbol
            && PositionGetInteger(POSITION_MAGIC) == magicNumber)
           {
            ...
            MqlDateTime dtNow;
            TimeToStruct(nowTime, dtNow);
            //--- opened the same calendar day as the current M1 bar
            if(dtOpen.day == dtNow.day
               && dtOpen.mon == dtNow.mon
               && dtOpen.year == dtNow.year)
               return(true);
           }
        }
     }
...
//| rejected (returns 0) if outside [volume_min, volume_max]         |
//+------------------------------------------------------------------+
