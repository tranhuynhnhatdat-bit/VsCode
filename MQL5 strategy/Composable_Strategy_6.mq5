...
//|            "2,4".                                                |
//|                                                                  |
//| Ichimoku parity: engine hardcodes senkou span = 52 (ignores      |
//| ichi_senkou). Handle = iIchimoku(symbol, H1, 9, 30, 52).         |
//| Senkou B is buffer MODE_SENKOUSPANB (1). MQL5 shifts it forward  |
//| by kijun, matching the engine's shifted SenkouB.                 |
//| Crosses above: prev Close <= prev SenkouB AND cur Close >        |
//| cur SenkouB.                                                     |
//+------------------------------------------------------------------+
#property copyright "Composable Strategy 6"
...
//+------------------------------------------------------------------+
// -- Inputs (defaults = strategy_6 params)
//+------------------------------------------------------------------+
input double InpRiskMoney    = 100.0;   // Risk money per trade (USD)
input double InpSlAtr        = 5.0;     // SL = InpSlAtr * ATR(InpAtrPeriod)
input int    InpAtrPeriod    = 39;      // ATR period (H1)
input int    InpIchiTenkan   = 9;       // Ichimoku tenkan period
input int    InpIchiKijun    = 30;      // Ichimoku kijun period
input int    InpIchiSenkou   = 52;      // Ichimoku senkou span (engine hardcodes 52)
...
input int    InpMagicNumber  = 6;       // Magic number
...
#define ICHI_1 1     // iIchimoku(_Symbol, PERIOD_H1, InpIchiTenkan, InpIchiKijun, InpIchiSenkou)
#define ICHI_SENKOUB 1 // Senkou Span B buffer (MODE_SENKOUSPANB = 1)
...
int OnInit()
  {
   ...
   //--- Create the built-in ATR + Ichimoku handles on H1
   if(!initIndicators())
      return(INIT_FAILED);
...
void OnTick()
  {
   ...
   //--- Condition: (Close < Open) OR (Close crosses above SenkouB) -> BUY.
   if(hour == InpEntryHour + 1)
     {
      ...
      //--- Condition B (or): Close crosses above SenkouB(9,30,52).
      bool condCrossSenkouB = false;
      double senkouBCur  = sqGetIndicatorBufferValue(ICHI_1, ICHI_SENKOUB, h1Shift);
      double senkouBPrev = sqGetIndicatorBufferValue(ICHI_1, ICHI_SENKOUB, h1Shift + 1);
      double closePrev = iClose(_Symbol, PERIOD_H1, h1Shift + 1);
      if(senkouBCur > 0.0 && senkouBPrev > 0.0 && closePrev > 0.0)
        {
         // crosses_above: prev Close <= prev SenkouB AND cur Close > cur SenkouB
         if(closePrev <= senkouBPrev && h1Close > senkouBCur)
            condCrossSenkouB = true;
        }

      //--- OR combination; if neither holds, no entry.
      if(!condCloseBelowOpen && !condCrossSenkouB)
         return;

      //--- SL distance = InpSlAtr * ATR(39) on that H1 bar
      ...
      //--- Send BUY
      if(!g_trade.Buy(size, _Symbol, ask, slPrice, 0.0, "Composable_6"))
...
bool initIndicators()
  {
   ...
   indicatorHandles[ICHI_1] = iIchimoku(_Symbol, PERIOD_H1,
                                        InpIchiTenkan, InpIchiKijun, InpIchiSenkou);
