...
//|   SL     : 3.0 x ATR(41) H1, SL = ask - distance (fixed)         |
...
//|            "2,4".                                                |
//|                                                                  |
//| Timeframe-agnostic: reads the current forming M1 bar via         |
//| iTime(_Symbol, PERIOD_M1, 0). Run in the Strategy Tester on ANY  |
//| period (M1 or H1) with XAUUSD.                                   |
//+------------------------------------------------------------------+
#property copyright "Composable Strategy 0"
...
void OnTick()
  {
   //--- Do we have enough history for ATR(41) on H1?
   ...
   if(hour == InpEntryHour + 1)
     {
      ...
      //--- SL distance = InpSlAtr * ATR(41) on that H1 bar
      ...
      //--- Send BUY
      if(!g_trade.Buy(size, _Symbol, ask, slPrice, 0.0, "Composable_0"))
