...
//| 
//|    Backtested on GBPJPY_ftmo / H1, 2003.08.04 - 2026.05.21 
//+------------------------------------------------------------------+
...
input string CustomComment = "GBPJPY";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 5;        //MagicNumber
...
input int Period2 = 5;        //Period2
input int ATRRisingPeriod1 = 99;        //ATRRisingPeriod1
input double StopLossCoef1 = 5;        //StopLossCoef1
input double StopLossCoef2 = 3;        //StopLossCoef2
...
#define LOWEST_1 0     //iCustom(NULL,0, "SqLowest", Period1, PRICE_OPEN)
#define LOWEST_2 1     //iCustom(NULL,0, "SqLowest", Period2, PRICE_WEIGHTED)
#define ATR_1 2     //iCustom(NULL,0, "SqATR", ATRRisingPeriod1)
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = ((((sqTimeHour(sqTime(NULL,0, 0)) == 11)
      &&   (sqTimeDayOfWeek(sqTime(NULL,0, 0)) == 3))
      &&   (((NormalizeDouble((double) sqGetIndicatorValue(LOWEST_1, 0, 3), 6) < NormalizeDouble((double) sqLow(NULL,0, 1), 6))
      &&   (NormalizeDouble((double) sqGetBid("Current"), 6) > NormalizeDouble((double) sqLow(NULL,0, 2), 6)))
      &&   (NormalizeDouble((double) sqGetIndicatorValue(LOWEST_2, 0, 3), 6) < NormalizeDouble((double) sqHigh(NULL,0, 2), 6))))
      ||    (((sqTimeHour(sqTime(NULL,0, 0)) == 12)
      &&   (sqTimeDayOfWeek(sqTime(NULL,0, 0)) == 2))
      &&   ((sqIsLowerCount("sqOpen(NULL,0, 1)","sqOpen(NULL,0, 2)",10,true,2))
      &&   (NormalizeDouble((double) sqClose(NULL,0, 3), 6) != NormalizeDouble((double) sqOpen(NULL,0, 2), 6)))));

     ShortEntrySignal = (((sqTimeHour(sqTime(NULL,0, 0)) == 4)
      &&   (sqTimeDayOfWeek(sqTime(NULL,0, 0)) == 5))
      &&   sqIsRising(ATR_1, 2, false, 1));

     LongExitSignal = (((sqTimeHour(sqTime(NULL,0, 0)) == 2)
      &&   (sqTimeDayOfWeek(sqTime(NULL,0, 0)) == 4))
      ||    ((sqTimeHour(sqTime(NULL,0, 0)) == 2)
      &&   (sqTimeDayOfWeek(sqTime(NULL,0, 0)) == 3)));

     ShortExitSignal = (sqTimeHour(sqTime(NULL,0, 0)) == 17);
...
//------------------------
...
)
   {
      ...
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL, openPrice, 2, StopLossCoef2 * sqGetIndicatorValue(ATR_2, 1)), "Current");
      pt = sqFixMarketPrice(0, "Current");
...
bool initIndicators(){
   ...
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[LOWEST_2] = iCustom(NULL,0, "SqLowest", Period2, PRICE_WEIGHTED);
...
double sqGetExpressionByIdentification(string id, int shift) {
      if(id == "sqOpen(NULL,0, 1)") {
          return   sqOpen(NULL,0, shift + 1);
      }
      if(id == "sqOpen(NULL,0, 2)") {
          return   sqOpen(NULL,0, shift + 2);
      }
